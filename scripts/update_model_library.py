...
if __name__ == "__main__":
    ...
    alphas = ["1.0", "1.2", "1.23", "1.25", "1.27", "1.3", "1.4"]
    ...
    data = pd.DataFrame(columns=["alpha", "mass", "folder_path", "model_name"])

    missing = {}
    did_not_explode = {}

    print(f"Updating model library with {len(alphas) * len(masses)} models...")
    for alpha, mass in track(itertools.product(alphas, masses), total=len(alphas) * len(masses), description=""):

        base_path = f"/mnt/research/SNAPhU/STIR/run_sukhbold/run_14may19_a{alpha}/run_{mass}"
        model_name = f"stir2_14may19_s{mass}_alpha{alpha}"

        if os.path.exists(base_path + "/" + model_name + ".dat"):
            ...
            if explosion_energy[-1] <= 0:
                print(f"DID NOT EXPLODE: alpha={alpha}, mass={mass}")
                if f"alpha {alpha}" not in did_not_explode: 
                    did_not_explode[f"alpha {alpha}"] = []
                did_not_explode[f"alpha {alpha}"].append(mass)

            else:
                print(f"EXPLODED: alpha={alpha}, mass={mass}")
                new_row = pd.DataFrame([[alpha, mass, base_path, model_name]], columns=["alpha", "mass", "folder_path", "model_name"])
                data = pd.concat([data, new_row]).reset_index(drop=True)

        else:
            print(f"MISSING: alpha={alpha}, mass={mass}")
            if f"alpha {alpha}" not in missing: 
                missing[f"alpha {alpha}"] = []
            missing[f"alpha {alpha}"].append(mass)

    data.to_csv(f"{config.root_path}/data/model_library.csv")
    print("Model library has finished updating.")
    print(f"Missing Models: {missing}" if len(missing) > 0 else "No missing models!")
    print(f"Models that did not explode: {did_not_explode}")