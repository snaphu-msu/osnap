"""
Plots the shock temperature vs. mass for a given run date and alpha.
"""
...
for mass in masses:
    ...
    if explosion_energy[-1] <= 0:
        print(f"The {mass} ZAMS mass model did not explode.")
        continue
    ...
    # Find the temperature just before the shock radius
    radius = stir_data['flash', 'r'].value
    shock_index = int(np.where(radius >= shock_radius[-1])[0][0])
    temps.append(stir_data['flash', 'temp'].value[shock_index - 1])
    successful_masses.append(float(mass))

plt.scatter(successful_masses, temps, s=5)
