...
def main():
    parser = argparse.ArgumentParser(
        description="Compute nucleosynthesis"
    )
    ...
    points = np.load("trajectories_" + model + ".npy")
    ncomps = 686
    if True:
        ...
        for j in range(num_points):
            ...
            filebase = "./comps/tardis_final_" + model + str(pID)
        ...
        np.save("tardis_final_" + model + "_comps.npy", fcomps)
    else:
        fcomps = np.load("tardis_final_" + model + "_comps.npy")
    ...
    np.save(model + "_totalmasses.npy", totalmasses)
    print(totalmasses)
